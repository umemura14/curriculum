...
-- [回答]
SELECT store_name,
goods_name,quantity FROM stock_table JOIN store_table ON stock_table.store_code =
store_table.store_code JOIN goods_table ON stock_table.goods_code = goods_table.goods_code;
...
VALUES('M001','マフラー','4500','2021-01-16');
 
 
-- 7. 在庫テーブルの商品コード='S987'、かつ、店舗コード='EA01'に対して、「在庫数=10、更新日付=本日日付」で更新しなさい。※実行後のSELECT結果も貼付すること。
-- [回答]
UPDATE stock_table SET quantity ='10', update_day='2021-01-18' 
